...
**Key Parameters**: - `width`, `height`, `units`: Always specify the physical size required by the journal. - `dpi`: Crucial for raster formats. - `device`: Can be used to specify the graphics device (e.g., `cairo_pdf` for better font embedding).
...
## Capstone Project: A Walkthrough

Let's apply everything we've learned to create a final, publication-ready, multi-panel figure.

![](assets/figs/capstone_figure.png)

::: {.callout-note collapse="true"}
### Code for the Capstone Figure
...
col_anno <- HeatmapAnnotation(
  compound = data$anno$compound, time = data$anno$time,
  col = list(compound = c("none" = "steelblue", "sucrose" = "tomato"),
             time = c("3 hour" = "lightgrey", "24 hour" = "darkgrey"))
)
ht <- Heatmap(mat_top_var, name = "Z-Score", top_annotation = col_anno,
              show_row_names = FALSE, column_title = "Top 50 Most Variable Genes")
...
p_volcano <- ggplot(data$diff, aes(x = sucrose_24h_lfc, y = -log10(sucrose_24h_pval))) +
  ...
  labs(title = "Volcano Plot (24h)", x = "Log2 Fold Change", y = "-log10(p-value)") +
  theme(legend.position = "none")

# Part C: Boxplot
top_gene_24h <- data$diff %>% filter(sucrose_24h_pval < 0.05) %>%
  arrange(sucrose_24h_pval) %>% pull(Gene_ID) %>% head(1)
plot_data <- data.frame(expression = as.numeric(mat[top_gene_24h, ]),
                        compound = data$anno$compound, time = data$anno$time)
p_boxplot <- ggplot(plot_data, aes(x = compound, y = expression, fill = time)) +
  geom_boxplot() + theme_bw() +
  labs(title = paste("Expression of", top_gene_24h), x = "Compound", y = "Normalized Expression") +
  scale_fill_manual(values = c("lightgrey", "darkgrey"))

# Assemble and Save
ht_grob <- grid.grabExpr(draw(ht))
# In the exercise, we save this to PDF and TIFF.
```
:::
