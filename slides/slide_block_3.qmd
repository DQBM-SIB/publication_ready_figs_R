...
- **`ggplotly()`**: A function from the `plotly` package that transforms a `ggplot` object into an interactive plot.
- Great for exploring data, as you can hover over points to see values, zoom in and out, and pan around.
...
library(plotly)
cars <- mtcars
cars$gear <- as.factor(cars$gear) # factor looks nicer than character
my_plot <- ggplot(cars, aes(x = wt, y = mpg, size = hp, color = gear, text = paste("Car:", rownames(cars), "\nHorsepower:", hp, "\nGear:", gear))) +
  geom_point(alpha = 0.7) + # semi-transparent points
  scale_color_brewer(palette = "Dark2") + # more distinct color palette
  scale_size_continuous(range = c(5, 15)) + # larger range for better horsepower differentiation
  labs(
    title = "Fuel Efficiency vs Weight of Cars",
    subtitle = "Size of points represents horsepower, color indicates gear",
    x = "Weight (1000 lbs)",
    y = "Miles per Gallon",
    color = "Gear",
    size = "Horsepower"
...
ggplotly(my_plot, tooltip = c("text", "x", "y", "size", "color"))
```
